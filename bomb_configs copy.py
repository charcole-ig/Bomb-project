...
# Define the characters and their borough orders (Phase 1)
# 1: Bronx, 2: Brooklyn, 3: Queens, 4: Staten Island
CHARACTERS = {
    "The Conductor": {"order": [1, 2, 3, 4], "wire": "red"},
    "The Engineer": {"order": [2, 1, 3, 4], "wire": "blue"},
    "The Student": {"order": [4, 2, 1, 3], "wire": "orange"}, # Knicks colors
    "The Inspector": {"order": [2, 1, 4, 3], "wire": "yellow"} # Taxi color
}

# Select a random character for this "boot"
active_char_name = choice(list(CHARACTERS.keys()))
active_char_data = CHARACTERS[active_char_name]

def genSerial():
    return "NYE-2026"

def genTogglesTarget():
    # The target is the list of toggles in the specific character's order
    return active_char_data["order"]

def genKeypadTarget():
    # "The number they are chanting in the streets" -> 2027 (New Year's)
    return "2027"

def genWiresTarget():
    # Map colors to wire indices (0-4) based on your hardware setup
    # Let's assume: 0:Red, 1:Blue, 2:Green, 3:Yellow, 4:Orange
    color_map = {"red": 0, "blue": 1, "yellow": 3, "orange": 4}
    return color_map[active_char_data["wire"]]

# Update your boot_text to show the backstory
boot_text = f"STATION: ONE TIMES SQUARE\n"\
            f"THREAT: THE CONDUCTOR'S CELL\n"\
            f"DISPATCHING AGENT RAMOS...\n"\
            f"INTEL: {active_char_name} has been spotted."
