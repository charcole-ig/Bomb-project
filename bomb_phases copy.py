...
class Toggles(PhaseThread):
    ...
    def run(self):
        self._running = True
        last_state = [False] * len(self._component)
        
        while (self._running):
            for i, pin in enumerate(self._component):
                # Detect a "flip" (from False to True)
                if pin.value == True and last_state[i] == False:
                    self._current_sequence.append(i + 1) # i+1 matches 1-4 borough logic
                    
                    # Check if the sequence matches the target so far
                    step = len(self._current_sequence) - 1
                    if self._current_sequence[step] != self._target[step]:
                        self._failed = True # Strike!
                        self._current_sequence = [] # Reset sequence
                    
                    if len(self._current_sequence) == len(self._target):
                        self._defused = True
                
                last_state[i] = pin.value
            sleep(0.1)